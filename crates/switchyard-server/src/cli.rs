...
impl ServerArgs {
    ...
    fn into_runtime(self) -> ServerResult<(ServerRuntime, ServerRunOptions)> {
        let mut runtime = load_server_runtime(&self.config)?;
...
/// Loads the configured algorithms and starts the server. The stock CLI now owns
/// the fleet-readiness monitor lifecycle: when the config declares a
/// `[fleet_readiness]` section, the runtime carries a monitor sharing one
/// `SharedFleetState` with the server and `run` chooses the monitored lifecycle.
